    ...
    m_cellId (cellId)
{
  ...
  m_enableSegwSplitting = false;
}
// Get another socket //
EpcEnbApplication::EpcEnbApplication (Ptr<Socket> lteSocket, Ptr<Socket> s1uSocket, Ptr<Socket> xwSocket, Ipv4Address enbS1uAddress, Ipv4Address sgwS1uAddress, Ipv4Address xwAddress, uint16_t cellId)
  : m_lteSocket (lteSocket),
    m_s1uSocket (s1uSocket),
    m_xwSocket (xwSocket),    
    m_enbS1uAddress (enbS1uAddress),
    m_sgwS1uAddress (sgwS1uAddress),
    m_xwAddress (xwAddress),
    m_gtpuUdpPort (2152), // fixed by the standard
    m_s1SapUser (0),
    m_s1apSapMme (0),
    m_cellId (cellId)
{
  ...
  m_s1uSocket->SetRecvCallback (MakeCallback (&EpcEnbApplication::RecvFromS1uSocket, this));
  m_lteSocket->SetRecvCallback (MakeCallback (&EpcEnbApplication::RecvFromLteSocket, this));
  m_xwSocket->SetRecvCallback (MakeCallback (&EpcEnbApplication::RecvFromXwSocket, this));
  m_s1SapProvider = new MemberEpcEnbS1SapProvider<EpcEnbApplication> (this);
  m_s1apSapEnb = new MemberEpcS1apSapEnb<EpcEnbApplication> (this);
  m_enableSegwSplitting = true;
}
...
void 
EpcEnbApplication::RecvFromXwSocket (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this);  
...
EpcEnbApplication::RecvFromLteSocket (Ptr<Socket> socket)
{
  ...
  if(m_enableSegwSplitting){
    ...
    Ipv4Header ipHeader = ipv4Header;
    ipHeader.SetSource ("7.0.0.3");
    pCopy->AddHeader(ipHeader);
    packet = pCopy;
  }
  else{;}
...
EpcEnbApplication::RecvFromS1uSocket (Ptr<Socket> socket)
{
  ...
  pCopy->RemoveHeader (ipv4Header);
  Ipv4Header ipHeader = ipv4Header;
  ipHeader.SetDestination ("11.0.0.1");
  pCopy->AddHeader(ipHeader);
  
  if(m_enableSegwSplitting){
    SendToLteSocket (pCopy, it->second.m_rnti, it->second.m_bid); 
  }
  else{
...
EpcEnbApplication::SendToLteSocket (Ptr<Packet> packet, uint16_t rnti, uint8_t bid)
{
  ...
  if(m_enableSegwSplitting){
    m_xwSocket->SendTo(packet, 0, InetSocketAddress (m_xwAddress, 500));
  }
  else{
