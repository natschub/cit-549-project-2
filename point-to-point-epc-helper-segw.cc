...
#include <ns3/epc-segw-application.h>
  ...
  : m_gtpuUdpPort (2152)  // fixed by the standard
{
  ...
  m_xwIpv4AddressHelper.SetBase ("26.0.0.0","255.255.255.252");
  ...
  m_segw = CreateObject<Node> ();
  internet.Install (m_segw);

  Ptr<Socket> segwXwSocket = Socket::CreateSocket (m_segw, TypeId::LookupByName ("ns3::UdpSocketFactory"));
  retval = segwXwSocket->Bind (InetSocketAddress (Ipv4Address::GetAny (), 500));
  NS_ASSERT (retval == 0);

  m_tunDevice2 = CreateObject<VirtualNetDevice> ();
  // allow jumbo packets
  m_tunDevice2->SetAttribute ("Mtu", UintegerValue (30000));

  m_tunDevice2->SetAddress (Mac48Address::Allocate ()); 

  m_segw->AddDevice (m_tunDevice2);
  NetDeviceContainer tunDeviceContainer2;
  tunDeviceContainer2.Add (m_tunDevice2);
  
  // the TUN device is on the same subnet as the UEs, so when a packet
  // addressed to an UE arrives at the intenet to the WAN interface of
  // the SeGW it will be forwarded to the TUN device. 
  Ipv4InterfaceContainer tunDeviceIpv4IfContainer2 = m_ueAddressHelper.Assign (tunDeviceContainer2);  

  m_segwApp = CreateObject<EpcSegwApplication> (m_tunDevice2, segwXwSocket);
  m_segw->AddApplication (m_segwApp);
  
  m_tunDevice2->SetSendCallback (MakeCallback (&EpcSegwApplication::RecvFromTunDevice, m_segwApp));
...
PointToPointEpcHelper::DoDispose ()
{
  ...
  m_tunDevice = 0;
  m_tunDevice2->SetSendCallback (MakeNullCallback<bool, Ptr<Packet>, const Address&, const Address&, uint16_t> ());
  m_tunDevice2 = 0;
  m_sgwPgwApp = 0;
  m_segwApp = 0;  
  m_sgwPgw->Dispose ();
...
PointToPointEpcHelper::AddEnb (Ptr<Node> enb, Ptr<NetDevice> lteEnbNetDevice, uint16_t cellId)
{
  ...
  p2ph.SetDeviceAttribute ("Mtu", UintegerValue (m_s1uLinkMtu));
  p2ph.SetChannelAttribute ("Delay", TimeValue (m_s1uLinkDelay));  
  NetDeviceContainer enbSegwDevices = p2ph.Install (enb, m_segw);
  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after installing p2p dev: " << enb->GetObject<Ipv4> ()->GetNInterfaces ());  
  Ptr<NetDevice> enbXwDev = enbSegwDevices.Get (0);
  Ptr<NetDevice> segwXwDev = enbSegwDevices.Get (1);
  m_xwIpv4AddressHelper.NewNetwork ();
  Ipv4InterfaceContainer enbSegwIpIfaces = m_xwIpv4AddressHelper.Assign (enbSegwDevices);
  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after assigning Ipv4 addr to S1 dev: " << enb->GetObject<Ipv4> ()->GetNInterfaces ());
  
  Ipv4Address enbXwAddress = enbSegwIpIfaces.GetAddress (0);
  Ipv4Address segwXwAddress = enbSegwIpIfaces.GetAddress (1);

  // create S1-U socket for the ENB
  Ptr<Socket> enbXwSocket = Socket::CreateSocket (enb, TypeId::LookupByName ("ns3::UdpSocketFactory"));
  retval = enbXwSocket->Bind (InetSocketAddress (enbXwAddress, 500));
  NS_ASSERT (retval == 0);
  ...
  if(m_enableSegwSplitting){
    enbApp = CreateObject<EpcEnbApplication> (enbLteSocket, enbS1uSocket, enbXwSocket, enbAddress, sgwAddress, segwXwAddress, cellId);
  }
  else{
  ...
  m_segwApp->AddEnb (enbXwAddress, segwXwAddress);
}
...
Ptr<Node>
PointToPointEpcHelper::GetSegwNode ()
{
  return m_segw;
}
